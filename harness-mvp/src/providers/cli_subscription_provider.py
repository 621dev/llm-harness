...
# 구독 CLI 단발 완성 호출의 벽시계 타임아웃.
#
# **120초 → 420초로 올렸다**(2026-08-03). 실측 근거:
#
# - 8차 측정(k=6, 구독 백엔드 생성)의 시도당 지연은 **268~432초**(평균 344초)였다.
#   한 시도는 후보 2회 + judge 1회이고 judge는 gemini(빠름)라, **구독 1회당 200초를
#   넘는다.** 7차도 279~326초였다.
# - 그전에 측정에서 claude direct 조건이 **타임아웃으로 실패한 적이 있다** — 즉 120초는
#   이미 정상 작업을 죽이고 있었다.
# - 420초는 측정 스크립트가 우회용으로 쓰던 값이고, 그 값으로 8차 12시도가 **한 번도
#   타임아웃 없이** 끝났다. 추정이 아니라 통과 실적이 있는 숫자다.
#
# **왜 이렇게 느린가**: claude CLI는 프롬프트가 6토큰인 호출에도 입력 3만~10만 토큰을
# 쓴다(Claude Code 시스템 프롬프트·도구 정의를 매번 로드 — v21 §8 실측). 모델이 생각하는
# 시간이 아니라 런타임 로딩이 대부분이다. `--bare`로 줄일 것 같지만 OAuth/구독 인증까지
# 꺼져서 못 쓴다.
#
# **대가를 함께 알아둘 것**: 타임아웃은 `is_retryable` 기본값(True)에 걸려 1회 재시도된다
# (`MAX_RETRIES=1`). 즉 **진짜로 멈춘 호출은 최악의 경우 840초를 쓴다.** 그래도 올리는 게
# 맞다고 판단한 이유: 낮은 타임아웃은 **정상 작업을 실패로 바꾸고 구독 호출을 버린다**
# (실패한 시도도 한도를 소모한다). 폭주 방어는 타임아웃이 아니라 `budget_subscription_calls`
# 가 담당한다.
