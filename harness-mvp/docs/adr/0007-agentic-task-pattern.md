...
### 재검증은 스크립트로 (2026-07-27 추가)

경계를 실제로 강제하는 게 CLI인 이상 "CLI 올린 뒤 다시 확인할 것"이라는 메모만
남기면 언젠가 잊는다. 위 프로브를 `scripts/verify_agent_boundary.py`로 고정했다 —
Bash 차단 / 워크스페이스 밖 읽기 차단 / **안쪽 쓰기 정상**(과차단 방지) 3종을
돌리고 종료 코드로 판정한다. 실제 CLI 호출이라 pytest 밖에 둔다.

```bash
PYTHONPATH=src python scripts/verify_agent_boundary.py
```

이 스크립트를 만들면서 **또 하나를 실측으로 발견했다**: 워크스페이스 경로가
Windows 8.3 단축 경로(`C:\Users\GSITM-~1\...`)면 CLI가 정규화한 cwd와 에이전트가
쓰는 절대경로가 문자열로 안 맞아 **정상적인 안쪽 쓰기까지 거부된다**(경계가
뚫리는 게 아니라 반대로 과차단). `agent_runner.agent_workspace()`가 경로를
`resolve()`하도록 고쳤다 — 경계 검증은 "못 하게 막혔나"뿐 아니라 "할 수 있어야
할 걸 막지는 않았나"도 봐야 한다는 걸 보여준 사례다.
...
- **경계는 CLI 구현에 의존한다**: 우리가 강제하는 건 CLI 인자일 뿐, 실제 차단은
  ...
  별도 사용자 계정)는 없다. CLI 업그레이드 후에는
  `scripts/verify_agent_boundary.py`로 반드시 재검증할 것.
...
- **재시도 없음**: 에이전트가 시작조차 못 하면 재시도하지 않는다 — 부분적으로
  ...
  "1회 재시도"와 다른 이유).
- **구독 사용량은 금액이 아니라 횟수로 집계한다**(2026-07-27 보완): `cost_usd`는
  구독이라 여전히 None이지만, `metrics.json`의 `subscription_calls`에 호출 횟수가
  남는다(에이전트는 턴 수 = 호출 수). 금액과 한도는 다른 자원이라 합칠 수 없어서
  따로 센다 — Section 9 Cost Blindness 방지.
