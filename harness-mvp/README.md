...
python -m pytest tests/ -v                                # 410개, 전부 mock — 실제 CLI/API 미호출
...
4. **구독 CLI 타임아웃(`DEFAULT_TIMEOUT_SEC`, 현재 120초) 조정 검토** — 2026-07-28
   claude로 측정을 돌렸을 때 조사형 프롬프트가 이 한도에 걸려 실패했다(재시도
   포함 평균 지연 260초). claude 백엔드로 측정하려면 선행 과제다. 에이전트 모드는
   별도 상한(`DEFAULT_AGENT_TIMEOUT_SEC` 600초)이라 문제되는 건 단발 완성 호출 경로.
...
`scripts/verify_agent_boundary.py` — **agentic_task 안전 경계 재검증**(2026-07-27,
ADR 0007). 경계를 실제로 강제하는 건 우리 코드가 아니라 claude CLI라, CLI가
인자를 해석하는 방식이 바뀌면 경계가 조용히 뚫린다(실제로 한 번 뚫렸다). 프로브
3종(Bash 차단 / 워크스페이스 밖 읽기 차단 / **안쪽 쓰기 정상** — 과차단 방지)을
돌려 종료 코드로 판정한다. 실제 CLI 호출이라 `pytest tests/` 밖.
...
달라지면 "구조 효과"와 "모델 효과"가 섞여 비교가 성립하지 않는데, 경고만 붙여
저장하면 그 숫자가 나중에 인용될 위험이 있어서다(2026-07-28 실제로 그런 run이
...
| `test_learning.py` | 15 | **run 간 학습**(2026-07-29) — 핵심은 **자동 집계가 다음 run에 절대 주입되지 않는다**는 것(사람이 `learned.md`를 쓴 것만 반영). 관측 자동 기록, `kind` 필드로 예산 중단과 provider 실패 구분, 깨진 줄이 로그 전체를 못 읽게 하지 않음, 학습 실패가 run을 죽이지 않음, 주입 사본을 run에 남김(재현성), `cli learn` 커맨드 실제 호출(모듈 테스트만으로는 `_out` 오용 버그를 못 잡았다) |
