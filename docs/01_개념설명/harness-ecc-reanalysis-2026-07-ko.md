...
## 2. 예산 상한 — 개념 자체가 없었다 (2순위, **2026-07-29 도입 완료 → §7**)
...
## 6.5 전환 여부 판단은 별도 문서에

"엔진을 계속 쓸 것인가, ECC식으로 전환할 것인가"는 이 문서 범위 밖이다 —
`harness-vs-ecc-decision-2026-07-ko.md`에서 실측 데이터(1·2차 측정)와 패턴별
근거를 놓고 판단했다. 요지: **전환하지 않되, 엔진을 한 덩어리로 보지 말고 패턴별로
유지 여부를 따로 판단한다.** `fan_out_judge`가 비교 측정 0회라는 사실이 그 판단의
중심이다.
...
**예산 상한(§2)도 도입했다**(같은 날, 사용자 승인 후). `harness/budget.py` 신규 +
`config.json`의 `budget_usd`/`budget_subscription_calls`. 상한이 두 종류인 이유는
구독 provider가 `cost_usd=None`이라 금액 상한으로는 안 걸리기 때문이다. 기본값은
무제한이라 기존 동작이 그대로다(346개 테스트 무변경 통과로 확인). 여기서도 테스트가
구현 버그를 잡았다 — `0.09+0.01 < 0.10` 부동소수점 때문에 상한을 그냥 지나쳤다.
테스트 346 → **357개**.

즉 이 재분석에서 나온 항목은 **전부 처리됐다**: 채택 3건(재시도 분류, Provider 계약
테스트, 예산 상한), 나머지는 §3~§5에 제외 이유와 함께 남겼다.
